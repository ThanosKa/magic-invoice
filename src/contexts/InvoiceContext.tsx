...
export function InvoiceProvider({ children }: { children: React.ReactNode }) {
  ...
  const saveInvoice = useCallback(() => {
    ...
    const updated = [
      next,
      ...list.filter(
        (i) => i.invoiceNumber !== formValues.details.invoiceNumber
      ),
    ];
  ...
  const exportInvoice = useCallback(
    async (format: ExportFormat) => {
      ...
      const extension =
        format === "json" ? "json" : format === "csv" ? "csv" : "xml";
...
function reviveDates(payload: FormSchemaType): FormSchemaType {
  ...
  if (clone.details.invoiceDate)
    clone.details.invoiceDate = new Date(clone.details.invoiceDate);
  if (clone.details.dueDate)
    clone.details.dueDate = new Date(clone.details.dueDate);
  return clone;
}
