                ...
                <div className="flex justify-between items-start mb-12">
                    ...
                    <div className="text-right">
                        {details.invoiceLogo ? (
                            ...
                            <img src={details.invoiceLogo} alt={t('invoice.preview.logoAlt')} className="h-16 object-contain ml-auto" />
                        ) : (
                    ...
                    <div>
                        ...
                        <div className="font-medium cursor-pointer">
                            {details.invoiceDate && details.invoiceDate instanceof Date
                                ...
                                : (typeof details.invoiceDate === 'string' ? details.invoiceDate : t('invoice.form.details.pickDate'))}
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="text-xs uppercase tracking-wider font-bold text-slate-400 mb-1">
                        ...
                        <div className="font-medium">
                            {details.dueDate && details.dueDate instanceof Date
                                ...
                                : (typeof details.dueDate === 'string' ? details.dueDate : t('invoice.form.details.pickDate'))}
                        </div>
                    </div>
                </div>
                    ...
                    <Table className="text-left">
                        <TableHeader>
                            <TableRow className="border-b border-slate-200">
                                <TableHead className="py-3 font-semibold text-slate-900">{t('invoice.preview.description')}</TableHead>
                                <TableHead className="py-3 text-right font-semibold text-slate-900 w-24">{t('invoice.form.items.qtyLabel')}</TableHead>
                                <TableHead className="py-3 text-right font-semibold text-slate-900 w-32">{t('invoice.form.items.priceLabel')}</TableHead>
                                <TableHead className="py-3 text-right font-semibold text-slate-900 w-32">{t('invoice.preview.amountLabel')}</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {details.items?.map((item, i) => (
                                <TableRow key={i} className="border-b border-slate-100 last:border-0">
                                    <TableCell className="py-4">
                                        <div className="font-medium text-slate-900">{item.name || t('invoice.preview.itemPlaceholder')}</div>
                                        {item.description && <div className="text-slate-500 text-xs mt-1">{item.description}</div>}
                                    </TableCell>
                                    <TableCell className="py-4 text-right text-slate-600">{item.quantity}</TableCell>
                                    <TableCell className="py-4 text-right text-slate-600">
                                        {formatCurrency(Number(item.unitPrice || 0), details.currency, intlLocale)}
                                    </TableCell>
                                    <TableCell className="py-4 text-right font-medium text-slate-900">
                                        {formatCurrency(Number(item.total || 0), details.currency, intlLocale)}
                                    </TableCell>
                                </TableRow>
                            ))}
                            {(!details.items || details.items.length === 0) && (
                                <TableRow>
                                    <TableCell className="py-8 text-center text-slate-400" colSpan={4}>
                ...
                <div className="flex justify-end mb-12">
                    <div className="w-64 space-y-3">
                        <div className="flex justify-between text-slate-600">
                            ...
                            <span>{formatCurrency(Number(details.subTotal || 0), details.currency, intlLocale)}</span>
                        </div>
                        ...
                        {details.discountDetails?.enabled && (
                            <div className="flex justify-between text-emerald-600">
                                <span>
                                    ...
                                    {details.discountDetails.amountType === 'percentage' && ` (${details.discountDetails.amount}%)`}
                                </span>
                                <span>- {formatCurrency(
                ...
                <div className="grid grid-cols-2 gap-8 text-sm pt-8 border-t border-slate-200">
                    <div>
                        {details.paymentInformation && (
                            <div className="mb-4">
                                <h4 className="font-bold text-slate-900 mb-1">{t('invoice.form.payment.paymentInfoTitle')}</h4>
                                <p className="text-slate-600 whitespace-pre-wrap">{details.paymentInformation}</p>
                            </div>
                        )}
                        {details.termsConditions && (
                            <div>
                                <h4 className="font-bold text-slate-900 mb-1">{t('invoice.form.payment.termsLabel')}</h4>
                    ...
                    {details.notes && (
                        <div className="text-right">
                            <h4 className="font-bold text-slate-900 mb-1">{t('invoice.form.payment.notesLabel')}</h4>
